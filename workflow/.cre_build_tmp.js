...
var configSchema = exports_external.object({
  ...
  reserveManagerAddress: exports_external.string(),
  tokenAddress: exports_external.string(),
  usdcAddress: exports_external.string(),
  circuitBreakerAddress: exports_external.string(),
  chainSelectorName: exports_external.string(),
  gasLimit: exports_external.string().default("500000"),
  tier1ThresholdBps: exports_external.number().default(10150),
  tier2ThresholdBps: exports_external.number().default(10100),
  tier3ThresholdBps: exports_external.number().default(10050)
});
var erc20Abi = parseAbi([
  "function totalSupply() view returns (uint256)",
  "function balanceOf(address) view returns (uint256)"
]);
var reserveManagerAbi = parseAbi([
...
var readOnchainReserves = (runtime2, evmClient) => {
  const usdcAddress = runtime2.config.usdcAddress;
  const reserveManagerAddress = runtime2.config.reserveManagerAddress;
  ...
  return reserves * 1000000000000n;
};
var readOnchainSupply = (runtime2, evmClient) => {
  const tokenAddress = runtime2.config.tokenAddress;
  const callData = encodeFunctionData({
    abi: erc20Abi,
...
var classifyRatio = (ratioBps, config) => {
...
var submitOnchainReport = (runtime2, evmClient, ratioBps, tier) => {
  const triggerHalt = tier === TIER.CRITICAL;
  const reportData = encodeAbiParameters(parseAbiParameters("uint256 reserveRatioBps, uint8 tier, bool triggerHalt"), [ratioBps, Number(tier), triggerHalt]);
  ...
  const writeResult = evmClient.writeReport(runtime2, {
    receiver: runtime2.config.circuitBreakerAddress,
    report: signedReport,
...
var onHealthCheck = (runtime2) => {
  runtime2.log("═══════════════════════════════════════════════════════════");
  runtime2.log("[SAFEGUARD] Health-check started");
  runtime2.log("═══════════════════════════════════════════════════════════");
  const network248 = getNetwork({
    chainFamily: "evm",
    chainSelectorName: runtime2.config.chainSelectorName,
    isTestnet: runtime2.config.chainSelectorName.includes("testnet")
  });
  if (!network248) {
  ...
  runtime2.log("[SAFEGUARD] Step 1: Reading USDC reserves from blockchain...");
  const reserves = readOnchainReserves(runtime2, evmClient);
  runtime2.log(`[SAFEGUARD]   ✓ Reserves: ${reserves.toString()} wei (${Number(reserves) / 1000000000000000000} USDC)`);
  runtime2.log("[SAFEGUARD] Step 2: Reading USDS total supply...");
  const supply = readOnchainSupply(runtime2, evmClient);
  runtime2.log(`[SAFEGUARD]   ✓ Supply: ${supply.toString()} wei (${Number(supply) / 1000000000000000000} USDS)`);
  if (supply === 0n) {
    runtime2.log("[SAFEGUARD]   ⚠ Supply is zero – skipping this cycle");
    return "SKIP: zero supply";
  }
  const ratioBps = reserves * 10000n / supply;
  const ratioPercent = Number(ratioBps) / 100;
  runtime2.log(`[SAFEGUARD] Step 3: Reserve Ratio = ${ratioBps} bps (${ratioPercent}%)`);
  const { tier, label } = classifyRatio(ratioBps, runtime2.config);
  runtime2.log(`[SAFEGUARD] Step 4: Risk Tier = ${label}`);
  runtime2.log("[SAFEGUARD] Step 5: Submitting signed report to circuit breaker...");
  const txHash = submitOnchainReport(runtime2, evmClient, ratioBps, tier);
  runtime2.log(`[SAFEGUARD]   ✓ Transaction: ${txHash}`);
  const summary = `${label} | ${ratioPercent}% | tx:${txHash.slice(0, 10)}...`;
  runtime2.log("═══════════════════════════════════════════════════════════");
  runtime2.log(`[SAFEGUARD] ✅ Health-check complete: ${summary}`);
  runtime2.log("═══════════════════════════════════════════════════════════");
  return summary;
};
